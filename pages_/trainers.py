...
with st.form("new_trainer_form", clear_on_submit=True):
    ...
    time_slot = st.text_input("Time Slot", key="trainer_time_slot")
    if st.form_submit_button("Add Trainer", key="add_trainer_button"):
        if name:
            ok, _ = safe_action(lambda: trainers_service.create_trainer(conn, name, mobile, time_slot))
            if ok:
...
if not trainer_list:
    ...
    for trainer in trainer_list:
        if st.session_state.editing_trainer_id == trainer["id"]:
            ...
            edited_time_slot = edit_cols[2].text_input(
                "Time Slot", value=trainer["time_slot"] or "", key=f"edit_trainer_time_slot_{trainer['id']}", label_visibility="collapsed"
            )
            if edit_cols[3].button("Save", key=f"save_trainer_{trainer['id']}"):
                ok, _ = safe_action(
                    lambda t=trainer: trainers_service.update_trainer(
                        conn, t["id"], edited_name, edited_mobile, edited_time_slot
                    )
                )
                if ok:
                    st.session_state.editing_trainer_id = None
